...
begin
  ...
  begin
    ...
    t[i].Publisher    :=  readstring($'{i}. Издательство:');
  ...
  *)
    ...
    t[10].Publisher := 'Мгонь';
    t[10].IssueYear := 2020;
  
  /// Исходная Таблица
  writeln('+--------------------------------+----------------------+--------------+-------------+'); {32, 22, 14, 13}
  writeln('|            ФИО Автора          |    Название книги    | Издательство | Год издания |');
  writeln('+--------------------------------+----------------------+--------------+-------------+'); 
  
  for i := 1 to Size do
    writeln('|', t[i].FullName + ' '*(32 - t[i].FullName.Length), '|', t[i].BookName:22, '|', t[i].Publisher:14 , '|', t[i].IssueYear:13, '|');
  writeln('+--------------------------------+----------------------+----------------------------+');
  ...
  for i := Size-1 downto 1 do
    for j := 1 to i do
      if t[j].Publisher > t[j+1].Publisher then
        (t[j], t[j+1]) := (t[j+1], t[j]);
  ...
  writeln('|            ФИО Автора          |-----------------------------------------------|');
  writeln('|                                |     всего     |   до 2000 г.  | после 2000 г. |');
  writeln('+--------------------------------+-----------------------------------------------|');
  
  for i := 1 to Size do
  begin
    /// Если Автора нету в списке
    if not (t[i].FullName in used) then
      for j := i to Size do
      ...
      if not (t[i].FullName in used) then    
        writeln('|', t[i].FullName + ' '*(32 - t[i].FullName.Length), '|', AllBooks:15, '|', (AllBooks - Temp):15, '|', Temp:15, '|');
